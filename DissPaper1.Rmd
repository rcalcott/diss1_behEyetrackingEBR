---
title             : "Shifting attention among familiar versus novel stimuli: Divergence in behavioral performance and eye blink predictors"
shorttitle        : "Shifting attention among familiar and novel stimuli"
...
author_note: |
  ...
  Data and registration information for these experiments can be found on the Open Science Network (OSF) at https://osf.io/at42v/. Sample sizes and experimental methods for all experiments were registered, and a more precise preregistration including analysis plan for Experiment 4 was also posted on OSF prior to data analysis for this study. Since the initial preregistrations, we learned that some aspects of our analysis plan were not an ideal fit for the data and made changes accordingly. Deviations from the preregistration plan are noted in the manuscript. Also note that in the Preregistration, the names of the experiments are different: Study 1a is Experiment 1, Study 1b is Experiment 2, Study 1c is Experiment 3, and Study 2 is Experiment 4.
  
  We would like to thank Aidan Boese, Jeremy Eclarinal, Anthony Fager, Alexa Hill, Lisa Huynh, Michael Morrison and Brandon Pinkston for assistance with data collection. Additionally, we thank Jason Hubbard for technical assistance with task programming and eye tracking.
...
genericDataPathMac <- '~/Dropbox/Studies/Dissertation/output/osfData/Study'
...
list <- sapply(expNums, loadData, genericDataPath=genericDataPath, USE.NAMES=TRUE, simplify=FALSE)
...
It is important to note, however, that although there is much evidence for a link between striatal DA and EBR [e.g., @JongkeesColzato_2016], the specific nature of this relationship is still unknown, and as outlined above, is likely to be more complicated than a straightforward index of striatal DA levels. Thus, for the present paper, we are using EBR to test the conditions in which striatal DA function is relevant to WM updating; however we will not be able to draw any directional conclusions about the relationship between striatal DA tone and WM updating. Rather, by determining indicate how EBR predicts flexible WM updating in the presence versus absence of familiar stimuli, the results can, in conjunction with other work [e.g., @DreisbachEtAl_2005], delineate the specific contexts in which having high EBR facilitates versus undermines WM updating. The findings can then be compared with the relationship that would be predicted if EBR indexed, for example, tonic striatal DA levels, and in the case of divergence, can inform future work on the relationship between EBR and striatal DA function. 
...
On a given trial, there was a 50% chance that the distractor letter would be in the same category as the target (Congruent trials, i.e., both vowels), and a 50% chance that the distractor would be in the opposite category (Incongruent trials, i.e. one consonant, one vowel). Target and distractor letters were never identical on a given trial.  
...
Dependent variables were lnRT and error rate, and additionally probability of distractor fixation and the target saccade time for eye tracking analyses. Independent variables were Trial (1-4), Congruence (Congruent, Incongruent), Block Type (Perseveration-Inhibition, Pure Updating), and EBR. We note that at the time of preregistration, we had planned to analyze Trial as a categorical variable (Switch vs. Non-Switch trials); however we later learned that given the unpredictability of updating timing, performance asymptotes more slowly following a switch (Monsell et al., 2003), and thus it was a better fit to include Trial as a continuous variable. Additionally, we chose to exclude Trials 5 and 6 from analyses, so that all sets would have an equal number of trials in the analysis, and also because RTs on these later trials showed a pattern of increased RTs, perhaps because participants were anticipating an upcoming shift and were distracted.  
Because participants' gender has been known to influence the relationship between EBR and cognitive control, we entered Gender as an additional independent variable in EBR analyses. All continuous predictor variables (Trial, EBR) were centered prior to analyses. In the preregistration, we proposed testing quadratic effects of EBR on behavioral and eye tracking indices. However, these analyses did not yield a consistent pattern of results and introduced a high degree of variance into the data, so they were left out of this manuscript. As noted above, outliers at the single-trial level (i.e. RTs) were removed from analyses, whereas outliers were at the level of Subject (i.e. EBR) or condition (i.e. error rate) were winsorized, in order to minimize the effects of outliers on the models while also retaining as many subjects as possible in the analyses.  
...
getFormattedResults <- function(LMMresults){
  ...
  LMMresults$beta <- ifelse(LMMresults$beta<abs(.01), 
                            formatC(LMMresults$beta, format='e',digits=2),
                            round(LMMresults$beta, digits=2))
  LMMresults$se <- ifelse(LMMresults$se<abs(.01), 
...
figDat1.1.1 <- group_by(figDat1.1.1, Trial, Blocktype) %>%
...
p=ggplot(figDat1.1.1, aes(x=Trial, y=meanRT, shape=Blocktype))+
  geom_line(aes(x=Trial, y=meanRT, linetype=Blocktype)) +
  scale_linetype_discrete(name='Block Type') +
  scale_shape_discrete(name='Block Type') +
  labs(x='Trial', y='RT (ms)') +
  coord_cartesian(ylim=c(640,860))
p+theme_apa()+geom_pointrange(aes(ymin = meanploRT, ymax = meanphiRT))
```

**Error Rate.** The null model for error rate had an ICC of `r icc0.1.2`. In the full model, there was a significant effect of Congruence (*B* = `r results1.1.2$beta[4]`, *SE*=`r results1.1.2$se[4]`, *t*(`r results1.1.2$df[4]`)=`r results1.1.2$tval[4]` *p*=`r results1.1.2$pval[4]`), with participants making more errors on Incongruent versus Congruent trials. The only other effect of note was an interaction between Congruence and Block Type (*B* = `r results1.1.2$beta[7]`, *SE*=`r results1.1.2$se[7]`, *t*(`r results1.1.2$df[7]`)=`r results1.1.2$tval[7]` *p*=`r results1.1.2$pval[7]`). As shown in Figure\ \@ref(fig:figModelb), incongruence costs were larger on Perseveration-Inhibition blocks compared to Pure Updating blocks. All other main effects and interactions did not reach significance.
...
figDat1.1.2 <- group_by(figDat1.1.2, Blocktype, Congruent) %>%
...
limits = aes(ymax = meanphi, ymin=meanplo)
dodge = position_dodge(width=0.9)

s=ggplot(figDat1.1.2, aes(x = Blocktype, y = meanErate, fill = Congruent))+
  geom_bar(stat='identity', position=dodge)+
  geom_errorbar(limits, position=dodge, width=0)+
  scale_fill_manual(values=c('grey53','grey25'),
                    name="Congruence",
...
```{r figModelc, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelc)'}
...
(ref:capEBRresults) Model estimates of reaction time (RT) as a function of EBR across different Block Types in A) Experiment 1 and B) Experiment 4. Error bars represent the 95% confidence interval (CI). The interaction between EBR and Block Type indicates that the RT cost of the Perseveration-Inhibition blocks is greater for subjects with higher EBR.

```{r getEBRfigs, echo=FALSE, out.width='80%', fig.cap='(ref:capEBRresults)'}

knitr::include_graphics('EBRresults.png')

```

**RT.** EBR was examined to determine the potential involvement of striatal DA in updating of WM representations across different contexts. There was no main effect of EBR on RT (*t*(`r results5.1.1$df[5]`)=`r results5.1.1$tval[5]` *p*=*n.s.*); however there was an interaction between EBR and Block Type (*B* = `r results5.1.1$beta[10]`, *SE*=`r results5.1.1$se[10]`, *t*(`r results5.1.1$df[10]`)=`r results5.1.1$tval[10]` *p*=`r results5.1.1$pval[10]`). As shown in Figure\ \@ref(fig:getEBRfigs)A, higher EBR predicted a greater gap in RT between the slower Perseveration-Inhibition and the relatively faster Pure Updating blocks.  All other effects involving EBR did not reach significance.  
...
```{r figModeld, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModeld)'}
...
```{r figModelf, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelf)'}
...
figDat1.2.1 <- group_by(figDat1.2.1, Trial, switchSet, Blocktype) %>%
  summarise(meanRT=mean(RT, na.rm=TRUE),
            meanploRT=mean(ploRT, na.rm=TRUE),
            meanphiRT=mean(phiRT, na.rm=TRUE),
            meantloRT=mean(tloRT, na.rm=TRUE),
...
p=ggplot(figDat1.2.1, aes(x=Trial, y=meanRT, shape=switchSet))+
  geom_line(aes(x=Trial, y=meanRT, linetype=switchSet)) +
  facet_grid(.~Blocktype) +
...
(ref:capRTresults) Model estimates of reaction time (RT) across conditions of the attention shifting task in A) Experiment 2, B) Experiment 3, and C) Experiment 4. Error bars represent the 95% confidence interval (CI). There was a significant interaction between Block Type, Set Type, and Trial, indicating that on Switch Sets, RTs were slower to asymptote on Perseveration-Inhibition blocks compared to Pure Updating blocks.


```{r getRTfigs, echo=FALSE, out.width='100%', fig.cap='(ref:capRTresults)'}

knitr::include_graphics('RTresults.png')
...
```{r figModelg, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelg)'}
...
figDat1.2.2 <- group_by(figDat1.2.2, Blocktype, Congruent, switchSet) %>%
  summarise(meanErate=mean(eRate, na.rm=TRUE),
            meanplo=mean(plo, na.rm=TRUE),
            meanphi=mean(phi, na.rm=TRUE),
            meantlo=mean(tlo, na.rm=TRUE),
...
levels(figDat1.2.2$switchSet) <- gsub(" ", "\n", levels(figDat1.2.2$switchSet))


limits = aes(ymax = meanphi, ymin=meanplo)
dodge = position_dodge(width=0.9)

s=ggplot(figDat1.2.2, aes(x = switchSet, y = meanErate, fill = Congruent))+
  geom_bar(stat='identity', position=dodge)+
  facet_grid(.~Blocktype) +
  geom_errorbar(limits, position=dodge, width=0)+
  scale_fill_manual(values=c('grey53','grey25'),
...
(ref:capERresults) Model estimates of error rate across conditions of the attention shifting task in A) Experiment 2, B) Experiment 3, and C) Experiment 4. Error bars represent the 95% confidence interval (CI). There was a significant interaction between Block Type, Set Type, and Congruence, such that participants made more errors on Incongruent trials of Perseveration-Inhibition blocks, on Sets in which the target color has recently switched.

```{r getERfigs, echo=FALSE, out.width='100%', fig.cap='(ref:capERresults)'}

knitr::include_graphics('ERresults.png')
...
```{r figModelh, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelh)'}
...
## Procedure
The procedure was identical to that for Experiment 2, except that participants performed the task without eyetracking, and in a separate room from the experimenter. Additionally, because of the shortened ITI, the experiment was briefer, lasting only approximately 1 hour.
...
```{r figModeli, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModeli)'}

figDat1.3.1 <- getFigDat_RT(model1.3.1)
figDat1.3.1 <- convertFigValues_RT(figDat1.3.1)

figDat1.3.1 <- group_by(figDat1.3.1, Trial, switchSet, Blocktype) %>%
  summarise(meanRT=mean(RT, na.rm=TRUE),
            meanploRT=mean(ploRT, na.rm=TRUE),
            meanphiRT=mean(phiRT, na.rm=TRUE),
            meantloRT=mean(tloRT, na.rm=TRUE),
...
p=ggplot(figDat1.3.1, aes(x=Trial, y=meanRT, shape=switchSet))+
  geom_line(aes(x=Trial, y=meanRT, linetype=switchSet)) +
  facet_grid(.~Blocktype) +
  scale_linetype_discrete(name='Set Type') +
  scale_shape_discrete(name='Set Type') +
...
```{r figModelj, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelj)'}

figDat1.3.2 <- getFigDat_ER(model1.3.2)

figDat1.3.2 <- convertFigValues_ER(figDat1.3.2)

figDat1.3.2 <- group_by(figDat1.3.2, Blocktype, Congruent, switchSet) %>%
  summarise(meanErate=mean(eRate, na.rm=TRUE),
            meanplo=mean(plo, na.rm=TRUE),
            meanphi=mean(phi, na.rm=TRUE),
            meantlo=mean(tlo, na.rm=TRUE),
...
levels(figDat1.3.2$switchSet) <- gsub(" ", "\n", levels(figDat1.3.2$switchSet))

limits = aes(ymax = meanphi, ymin=meanplo)
dodge = position_dodge(width=0.9)

s=ggplot(figDat1.3.2, aes(x = switchSet, y = meanErate, fill = Congruent))+
  geom_bar(stat='identity', position=dodge)+
  facet_grid(.~Blocktype) +
  geom_errorbar(limits, position=dodge, width=0)+
  scale_fill_manual(values=c('grey53','grey25'),
...
**Error Rate.** The null model had an ICC of `r icc0.3.2`. In the prediction model, unlike in the previous experiments, the main effect of Congruence did not reach significance (*B* = `r results1.3.2$beta[5]`, *SE*=`r results1.3.2$se[5]`, *t*(`r results1.3.2$df[5]`)=`r results1.3.2$tval[5]` *p*=`r results1.3.2$pval[5]`), however its direct was consistent with the prior experiments. The only other effect of note was the 3-way interaction between Block Type, Set Type, and Congruence (*B* = `r results1.3.2$beta[15]`, *SE*=`r results1.3.2$se[15]`, *t*(`r results1.3.2$df[15]`)=`r results1.3.2$tval[15]` *p*=`r results1.3.2$pval[15]`). This effect (Figure\ \@ref(fig:getERfigs)B) is consistent with the other experiments, and indicates that on Switch Sets in Perseveration-Inhibition blocks, there was a larger error cost of Incongruent trials, compared to Switch Sets in Pure Updating blocks.

# Discussion
Increasing the pace of the task in Experiment 3 led to results that were largely consistent with those of the previous experiments, with a few exceptions. In particular, participants did not respond more slowly overall on Perseveration-Inhibition blocks, nor did they make more errors overall on Incongruent trials, both of which have been observed in the other experiments. Thus, it is possible that these effects are dependent on a slower-paced task in which participants may be more prone to attentional lapses. On the other hand, the key 3-way interactions observed in the prior experiments were also present here: In the context of Switch Sets within Perseveration-Inhibition blocks, participants still showed a slower reduction in RTs across trials, and had higher incongruence costs in error rate. Thus, these latter effects are unlikely to be the result of the slower task pace in the prior experiments.

# Experiment 4
In this final experiment, we tested in a large sample whether the EBR-linked effects from Experiment 1 replicated after some of its limitations were addressed, by including Non-Switch cues and a longer EBR baseline period of 5 minutes. Additionally, Experiment 4 allowed us to test whether the findings of Experiment 2 replicate, and in particular those that diverged from Experiment 1. 
...
```{r figModelk, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelk)'}

figDat1.4.1 <- getFigDat_RT(model1.4.1)
figDat1.4.1 <- convertFigValues_RT(figDat1.4.1)

figDat1.4.1 <- group_by(figDat1.4.1, Trial, switchSet, Blocktype) %>%
  summarise(meanRT=mean(RT, na.rm=TRUE),
            meanploRT=mean(ploRT, na.rm=TRUE),
            meanphiRT=mean(phiRT, na.rm=TRUE),
            meantloRT=mean(tloRT, na.rm=TRUE),
...
p=ggplot(figDat1.4.1, aes(x=Trial, y=meanRT, shape=switchSet))+
  geom_line(aes(x=Trial, y=meanRT, linetype=switchSet)) +
  facet_grid(.~Blocktype) +
  scale_linetype_discrete(name='Set Type') +
  scale_shape_discrete(name='Set Type') +
...
```{r figModell, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModell)'}

figDat1.4.2 <- getFigDat_ER(model1.4.2)

figDat1.4.2 <- convertFigValues_ER(figDat1.4.2)

figDat1.4.2 <- group_by(figDat1.4.2, Blocktype, Congruent, switchSet) %>%
  summarise(meanErate=mean(eRate, na.rm=TRUE),
            meanplo=mean(plo, na.rm=TRUE),
            meanphi=mean(phi, na.rm=TRUE),
            meantlo=mean(tlo, na.rm=TRUE),
...
levels(figDat1.4.2$switchSet) <- gsub(" ", "\n", levels(figDat1.4.2$switchSet))


limits = aes(ymax = meanphi, ymin=meanplo)
dodge = position_dodge(width=0.9)

s=ggplot(figDat1.4.2, aes(x = switchSet, y = meanErate, fill = Congruent))+
  geom_bar(stat='identity', position=dodge)+
  facet_grid(.~Blocktype) +
  geom_errorbar(limits, position=dodge, width=0)+
  scale_fill_manual(values=c('grey53','grey25'),
...
```{r figModelo, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelo)'}
...
```{r figModelp, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelp)'}
...
```{r figModelq, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelq)'}
...
```{r figModelr, echo=FALSE, message=FALSE, include=FALSE, out.width='100%', fig.cap='(ref:capModelr)'}
...
It is important to note, however, that the eye tracking results from Experiment 2 did not show this pattern of increased speed across trials on Switch sets of Pure Updating blocks. Given its relatively small sample size (*N* Exp. 2=`r Ngaze2`; *N* Exp. 4=`r Ngaze4`), it is possible that Experiment 2 was underpowered to detect these effects. An additional potential explanation is that in Experiment 2, stimuli were presented for only 1000ms, whereas in Experiment 4 stimuli remained on the screen until response. It is possible that this manipulation changed how participants responded to the task, although we note that the RT and Error Rate results were quite similar. Additionally, when trials in which the critical gaze measures (i.e. distractor fixation, saccade to target) occurred after 1000ms were removed, the results of Experiment 4 remained the same.  
